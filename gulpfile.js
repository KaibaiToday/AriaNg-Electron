...
gulp.task('prepare-aria2',()=>gulp.src('src/aria2/*').pipe(gulp.dest('./dist/aria2/')))
...
gulp.task('dev',gulp.series('clean','build','prepare-aria2',()=> {
    gulp.src('dist').pipe(exec('npx electron main.js dev')).pipe(exec.reporter({ 
      stdout: true, // 默认为 true，设置为 false 时不报告标准输出
      stderr: true, // 默认为 true，设置为 false 时不报告标准错误输出
      err: true     // 默认为 true，设置为 false 时不报告错误);
    }))
    gulp.watch('src/styles/**/*',gulp.series('process-assets',()=>{
        exec('npx electron reload')
    }));
    gulp.watch('src/*.html',gulp.series('process-assets',()=>{
        exec('npx electron reload')
    }));
}));

gulp.task('package',gulp.series(()=>gulp.src('dist').pipe(exec.reporter({ 
      stdout: true,
      stderr: true,
      err: true
...
exports.buildexe=gulp.series('clean','build','package','prepare-extraRes');